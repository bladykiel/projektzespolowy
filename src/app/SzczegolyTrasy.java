...
public class SzczegolyTrasy {
	float cena=0;
	   String nazwa1="";
	   String nazwa2="";
    SzczegolyTrasy(int idPolaczenia,int nr1Przystanku,int nr2Przystanku, String nazwaU, Date data){
    ...
    while (info.next()){
    	if(info.getInt("nr")==nr1Przystanku){
    		nazwa1=info.getString("name");
    	   }
    	if(info.getInt("nr")==nr2Przystanku){
    		nazwa2=info.getString("name"); 
    	   }
           JButton platnosc = new JButton("Kup bilet");
           platnosc.addActionListener(new ActionListener() {
           	public void actionPerformed(ActionEvent arg0) {
           		//new Komentarze(idPolaczenia);
           	//	nazwa(imie nazwisko), data przejazdu, Skad, Dokad, Cena
...
//new Platnosc(nazwaU,nazwa1,nazwa2,cena,data);
           ...
           platnosc.setBounds(229,1,150,25);
           platnosc.setBorder(new EmptyBorder(10, 10, 10, 10));
